...
describe('Event', () => {
  ...
  describe('Seed events', () => {
    ...
    describe('#PUT /api/v1/events/:eventId', () => {
      ...
      it.skip('should not update event if date and center is taken by another event by the same user', done => {
        request
          .post(loginApi)
          .send({
            email: mockData.admin.email,
        ...
        request
          ...
          .end((err, res) => {
            ...
            request
              .put('/api/v1/events/4')/////////////////////////////////
              .set({ 'x-access-token': token })
              .send({title: 'trying to update', notes: 'This update should fail', date: '2018-12-12'})
              .end((err, res) => {
                // expect(res.statusCode).to.equal(403);
                // expect(res.body.success).to.equal(false);
                expect(res.body.message).to.equal(
