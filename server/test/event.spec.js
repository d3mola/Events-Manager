...
describe('Event', () => {
  ...
  describe('Seed events', () => {
      ...
      it('should get all events created by this user', (done) => {
        request.post('/api/v1/users/login')
          ...
          .end((err, res) => {
            ...
            request.get('/api/v1/users/auth/events')
    ...
    describe('#GET /api/v1/users/auth/events/:eventId', () => {
      it('should get a single event as specified by id', (done) => {
        request.post('/api/v1/users/login')
          ...
          .end((err, res) => {
            const token = res.body.token;
            request.get('/api/v1/users/auth/events/2')
              .set({'x-access-token': token })
              .end((err, res) => {
                expect(res.status).to.equal(200);
                expect(res.body.success).to.equal(true);
      ...
      it('should not create an event if chosen center does not exist', (done) => {
        request.post('/api/v1/users/login')
          ...
          .end((err, res) => {
            ...
            request.post('/api/v1/events')
              .set({'x-access-token': token })
              .send({...mockData.newEvent, centerId: 90})
              .end((err, res) => {
    ...
    describe('#PUT /api/v1/events/:eventId', () => {
          ...
          .end((err, res) => {
            ...
            request.put('/api/v1/events/1')
              ...
              .end((err, res) => {
                expect(res.statusCode).to.equal(401);
                expect(res.body.success).to.equal(false);
                expect(res.body.message).to.equal('You\'re not authorized to access this route');
                done();
              })
          })
      });
      it.skip('should not update event if date and center is taken by another event', (done) => {
        request.post(loginApi)
          .send(mockData.admin)
          .end((err, res) => {
            const token = res.body.token;
      ...
      it('should update event if required fields are not supplied', (done) => {
        request.post(loginApi)
          .send(mockData.admin)
          .end((err, res) => {
            const token = res.body.token;
            request.put('/api/v1/events/1')
              ...
              .end((err, res) => {
                ...
                expect(res.body.updatedEvent.title).to.equal(mockData.birthday.title);
                done();
              })
          })
      });
    ...
    describe('#DELETE /api/v1/events/:eventId', () => {
      it('should not delete event if user isnt logged in', (done) => {
        request.delete('/api/v1/events/3')
          .end((err, res) => {
            expect(res.statusCode).to.equal(401);
            expect(res.body.success).to.equal(false);
            expect(res.body.message).to.equal('You need to login to access this route');
          ...
          .end((err, res) => {
            ...
            request.delete('/api/v1/events/2')
              ...
              .end((err, res) => {
                ...
                expect(res.body.success).to.equal(false);
                expect(res.body.message).to.equal('You cannot delete this event');
                done();
              })
          })
      });
      it('should not delete an event that doesnt exist', (done) => {
        ...
        .end((err, res) => {
          const token = res.body.token;
          request.delete('/api/v1/events/8')
            .set({'x-access-token': token})
            .end((err, res) => {
              expect(res.statusCode).to.equal(400);
              expect(res.body.success).to.equal(false);
